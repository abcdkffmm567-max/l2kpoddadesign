...
function toast(msg,type='success'){let t=document.querySelector('.toast');if(!t)return;t.textContent=msg;t.classList.remove('error');if(type==='error')t.classList.add('error');t.classList.remove('show');void t.offsetWidth;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2600)}
...
function renderHeroSlider(){
  const box=document.querySelector('#heroBannerSlider');
  ...
  box.innerHTML=heroSlides.map((url,i)=>`
    <div class="hero-slide ${i===heroSlideIndex?'active':''}" style="background-image:url('${url.replace(/'/g,"%27")}')"></div>
  `).join('');
...
function startHeroSlider(){
  ...
  heroSlideTimer=setInterval(()=>setHeroSlide(heroSlideIndex+1),4500);
}

function applyHeroBanner(){
  if(typeof db==='undefined')return;
  db.ref('site/heroBanners').on('value',snap=>{
    const raw=snap.val()||{};
    heroSlides=Object.keys(raw).sort().map(k=>raw[k]).filter(Boolean).slice(0,4);

    // backwards compatibility with the old single banner
    if(!heroSlides.length){
      db.ref('site/heroBannerUrl').once('value').then(oldSnap=>{
...
function applySharedSiteImages(){
  if(typeof db==='undefined') return;
  db.ref('site/images').on('value',snap=>{
    const v=snap.val()||{};
    const promo=document.querySelector('.ad-space');
    if(promo && v.promoBannerUrl){
      promo.style.background=`linear-gradient(rgba(4,10,24,.15),rgba(4,10,24,.15)),url("${v.promoBannerUrl}") center/cover no-repeat`;
      promo.textContent='';
    }
  });
}
...
document.addEventListener('DOMContentLoaded',()=>{
  setupNav();applyHeroBanner();applySharedSiteImages();renderServices();renderProducts(new URLSearchParams(location.search).get('cat')||'');renderPosts();
